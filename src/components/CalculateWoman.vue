<template>
  <div class="m-4">
    <h3 class="mb-4 text-center">Kadınlar için Kalori Hesaplama bölümüne hoş geldiniz! Aşağıdaki bilgileri doldurarak almanız gereken kaloriyi hesaplayabilirsiniz!</h3>
    <p>
...
export default {
  ...
  computed: {
    showYag(){
      return ((1.20 * (Number(this.weight) / ((Number(this.height)/100) * (Number(this.height)/100)))) + (0.23 * Number(this.age))) - 5.4
      },
    showLBM () {
    ...
    showOgun () {
      if (this.age > 18 && this.age < 20 && this.showYag > 2.0 && this.showYag < 3.9 ) {
        return "Ölmüşsün sen."
      } else {
        return "";
      }
